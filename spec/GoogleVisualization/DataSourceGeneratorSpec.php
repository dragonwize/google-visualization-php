...
class DataSourceGeneratorSpec extends ObjectBehavior
{
    ...
    {
        $inputData  = json_decode('[{"gender": "male"}, {"gender": "female"}]');
        $columns    = json_decode('[{"type": "string", "id": "gender"}]');
    ...
    {
        $inputData  = json_decode('[{"gender":"male", "age": 27}, {"gender":"female", "age": 24}]');
        $columns    = json_decode('[{"type": "string", "id": "gender"}, {"type": "number", "id": "age"}]');
        $outputData = $this->createCells($inputData[0], $columns);
        $outputData->shouldBeArray();
    ...
    function it_accepts_null_for_datetime_fields()
    {
        date_default_timezone_set('UTC');
        $inputData = json_decode('[{"date":null}]');
        $outputData = $this->generate($inputData, ["date" => "datetime"]);
        $outputData->rows->shouldHaveCount(1);
        $outputData->rows[0]->c[0]->v->shouldBeNull();
        $inputData = json_decode('[{"date":"null"}]');
        $outputData = $this->generate($inputData, ["date" => "datetime"]);
        $outputData->rows->shouldHaveCount(1);
        $outputData->rows[0]->c[0]->v->shouldBeNull();
